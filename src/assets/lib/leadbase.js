...
iScroll.prototype = {
    ...
    */
    ...
    refresh: function () {
        ...
        that.scrollerH = m.round((that.scroller.offsetHeight + that.minScrollY + pullupH) * that.scale) // 修复顶部距离bug for 利得金融
    ...
    var Base = {
        ...
        tracking : function(){
            ...
            switch ( _this.getEnv()) {
                case 'localhost':
                case 'dev':
                    action = 'http://10.1.1.119:8080/trace/test.jsp';
                    break;
                case 'test':
                    action = 'http://tracetest.leadfund.com.cn/trace/sendLog.jsp';
                    break;
                case 'pre':
                case 'pro':
            ...
            _data = [{
                terminal : option.terminal || _terminal,
        ...
        Pull : {
            ...
            scrollIt: function(parameter, pullDownEl, pullDownOffset, pullUpEl, pullUpOffset) {
                ...
                eval(parameter.id + "= new iScroll(parameter.id, {vScrollbar: true,topOffset: pullDownOffset,onRefresh: function () {_this.onRelease(pullDownEl,pullUpEl,this);},onScrollMove: function () {_this.onScrolling(this,pullDownEl,pullUpEl,pullUpOffset);},onScrollEnd: function () {_this.onPulling(pullDownEl,parameter.pullDownAction,pullUpEl,parameter.pullUpAction,this);},animation:this.animation})");
            ...
            onScrolling: function(e, pullDownEl, pullUpEl, pullUpOffset) {
                ...
                if( this.animation && e.y - e.maxScrollY <= 0 && e.y - e.minScrollY < 0) {
                    ...
                    if( Math.abs(e.y - e.minScrollY ) <= 50) {

                        this.animations( e.y - e.minScrollY );

                    } else {

                        if( Math.abs(e.y - e.maxScrollY ) <= 50 ) {
                            this.animations( e.y - e.maxScrollY );
                        } else {
                            _this.$pullUp.css('height', '100px');
            ...
            animations : function( ScrollY ){
                ...
                [2,3,4,5,6].forEach(function(i){
                     _this.$pullUp.removeClass('b' + i);
                });

                _this.$pullUp.css('height', 40 + Math.abs(ScrollY) + 'px');
            ...
            onRelease: function(pullDownEl, pullUpEl,e) {
                ...
                if (pullDownEl.className.match('loading')) {
                    ...
                    if(this.animation) {
                        this.$pullDown.removeClass('b6');
                        this.$pullDown.css('height', '45px');
                        this.$pullDown.addClass('b1');
                    };
                    pullDownEl.querySelector('.pullDownLabel').innerHTML = this.animation ? "" : _this.info.pullDownLable;
                };
...
;(function () {
    ...
    //open share font
    mutual.isOpenShareFont = function (shareId, isFont) {

        // 开启字号按钮
        if (( shareId == null || shareId == "" || typeof shareId == "undefined") && isFont != null) {
            mutual.jsCallNative({
        ...
        // 两个按钮都开启
        } else if (shareId != null && typeof shareId != "undefined" && isFont != null) {
            mutual.jsCallNative({
                'nativeName': 'common.webcontainer.navigation.right',
                'parameter': {
                    "items": [
        ...
        // 开启share按钮
        } else if (shareId != "" && typeof shareId != "undefined" && isFont == null) {
            mutual.jsCallNative({
                'nativeName': 'common.webcontainer.navigation.right',
                'parameter': {
                    "items": [
    ...
    //             }
    //         })
    //     } else if (typeof shareId == "undefined" && isFont != null) {
    //         mutual.jsCallNative({
    //             'nativeName': 'common.webcontainer.navigation.right',
    //             'parameter': {
    //                 "items": [
    ...
    //open share page
    mutual.openSharePage = function (shareId) {
        openURL('leadbank://page.ld/common.share?id=' + shareId);
    }
    //open login
